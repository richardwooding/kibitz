// The Hex service — the connection game on an 11×11 rhombus. Same shape as the
// Gomoku service (game.Table for seats/lifecycle, on-demand Start,
// both-sides-validate with a position hash computed identically on send and
// receive); the move is a free (row,col) placement. Hex never draws — exactly
// one player connects their two edges.
package hex
...
type snapshot struct {
	...
	Winner  int8     `cbor:"6,keyasint"` // 0 none, 1 red, 2 blue
...
type State struct {
	Playing  bool
	Board    Board
	P1ID     wire.ParticipantID // red, moves first, connects top↔bottom
	P2ID     wire.ParticipantID // blue, connects left↔right
	TurnID   wire.ParticipantID // 0 when over/idle
	Outcome  string             // "", "red wins", "blue wins"
	WinCells []int16            // the connecting group, for highlight
	Last     int16              // last placed stone's index, -1 when none
	History  []string           // ordered move notation, "🔴 e5" / "🔵 h9"
	Legal    []int8             // empty-cell indices (for the bot & UI)
...
var ErrNotTurn = errors.New("hex: not your turn")
...
type Service struct {
	...
	ph      game.Phase
	turn    game.Side
	winner  int8 // 0 in play, 1 red, 2 blue
	last    int16
	history []string
...
// (migration); the next joiner re-seeds the opponent via MemberKeyed.
...
// notePlaceLocked appends a move's notation ("🔴 e5" / "🔵 h9", column letter +
...
func (s *Service) Resign() error {
	...
	if !seated || s.ph != game.Playing {
		s.mu.Unlock()
		return errors.New("hex: no game to resign")
	}
...
func (s *Service) HandleFrame(from wire.ParticipantID, body []byte) error {
	...
	case kindNewGame:
		if from != s.ctx.HostID {
			return fmt.Errorf("hex: new game from non-host %d", from)
		}
		s.mu.Lock()
	...
	return fmt.Errorf("hex: unknown message kind %d", m.Kind)
}
...
func (s *Service) OfferTakeback() error {
	...
	if !s.canOfferLocked(s.ctx.Self) {
		s.mu.Unlock()
		return errors.New("hex: no takeback available")
	}
...
func (s *Service) AcceptTakeback() error {
	...
	if s.offerBy == 0 || s.offerBy == s.ctx.Self || s.ph != game.Playing {
		s.mu.Unlock()
		return errors.New("hex: no takeback to accept")
	}
	s.revertLocked()
	s.mu.Unlock()
...
func (s *Service) handlePlace(from wire.ParticipantID, m msg) error {
	...
	s.mu.Unlock()
	if !ok {
		return errors.New("hex: position hash mismatch")
	}
	s.emitState()
	return nil
}
...
func (s *Service) handleResign(from wire.ParticipantID) error {
	...
	if !seated || s.ph != game.Playing {
		s.mu.Unlock()
		return errors.New("hex: resign outside game")
	}
	s.winner = int8(side.Opponent()) + 1
	s.ph = game.Over
	s.mu.Unlock()
...
func (s *Service) Restore(blob []byte) error {
	...
	if err != nil {
		return fmt.Errorf("hex: restore: %w", err)
	}
	s.mu.Lock()
...
func (s *Service) checkTurnLocked(who wire.ParticipantID) (game.Side, error) {
	if s.ph != game.Playing {
		return 0, errors.New("hex: no game in progress")
	}
	side, seated := s.table.Seats.SideOf(who)
	if !seated {
		return 0, errors.New("hex: not a player")
	}
	if side != s.turn {
		return 0, ErrNotTurn
	}
...
// hash convention every open-information game shares. Hex has no draw: a
// non-winning move just passes the turn.
...
func (s *Service) stateLocked() State {
	...
	case s.ph == game.Over && s.winner == 2:
		st.Outcome = "blue wins"
	default:
		st.TurnID = s.table.Seats.IDOf(s.turn)
		st.Legal = s.legalLocked()
	}
