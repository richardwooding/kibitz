...
func TestGinDealAndPlay(t *testing.T) {
	...
	// Deal completes: both reach the draw phase holding 10 cards.
	hs := ginWait(t, host, func(s gin.State) bool { return s.Phase == "draw" && len(s.Hand) == 10 })
	ps := ginWait(t, player, func(s gin.State) bool { return s.Phase == "draw" && len(s.Hand) == 10 })
	...
	// It is the joiner's (P2) turn first.
	if hs.TurnID != player.client.Self() {
		t.Fatalf("first turn = %d, want joiner", hs.TurnID)
	}
